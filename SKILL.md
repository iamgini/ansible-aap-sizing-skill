---
name: ansible-aap-sizing-skill
description: Use when someone needs to size or right-size infrastructure for an Ansible Automation Platform deployment — mapping a use case (new greenfield deployment, migration from RPM to containerized, scaling an existing cluster, adding EDA, adding Private Automation Hub, OCP Operator deployment) to the topology, node types and counts, component resource specs, Automation Mesh design, and cost. Trigger on phrases like "how many nodes do we need", "what hardware for 500 managed hosts", "right-size AAP", "containerized vs OCP Operator", "how many execution nodes", "size EDA for 1000 events/s", "AAP on OpenShift resource limits", or any question about AAP topology (growth/enterprise/resilient) for a deployment.
---
